...
2. **Inventário** – `/inventory` lista vessels, strains e cartas fundidas (tabelas `user_inventory`, `user_strains`, `user_cards`). Dissolver devolve essência e faz hard delete; histórico em `audit_inventory_ledger` / `audit_strains_ledger`.
3. **Fusão** – Em `/fusion` o usuário escolhe 1 vessel + 1 strain. A action `fuseCardAction` usa Drizzle: hard delete dos itens consumidos, INSERT em `user_cards` com stats (previewFusion).
...
- `user_inventory` – vessels do usuário (referência a catalog). Hard delete quando consumido; snapshot em `audit_inventory_ledger`.
- `user_strains` – strains do usuário. Hard delete quando consumido; snapshot em `audit_strains_ledger`.
